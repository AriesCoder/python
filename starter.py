...
colors = ['white', 'red', 'blue', 'cyan', 'black']
...
print('array length: ', len(friends))
...
for row in open_file:
    split = row.rstrip('\n').split(',')
    # print(split)
    total_invoice = float(split[-1])* float(split[-2])
...
for row in open_file:
    split = row.rstrip('\n').split(',')
    total_invoice = float(split[-1])* float(split[-2])
    all_invoice = all_invoice + total_invoice
    all_invoice = round(all_invoice, 2)

print('all_invoice', all_invoice)
open_file.seek(0,0)
...
for row in open_file:
    if "Chocolate" in row:
        ...
        chocolate_invoice = chocolate_invoice + total
print(chocolate_invoice)
open_file.seek(0,0)   
...
for row in open_file:
    if "Vanilla" in row:
        row = row.rstrip('\n').split(',')
        total = float(row[-2])* float(row[-1])
        vannila_invoice = vannila_invoice + total
print(vannila_invoice)
open_file.seek(0,0)
...
for row in open_file:
    if "Strawberry" in row:
        row = row.rstrip('\n').split(',')
        total = float(row[-2])* float(row[-1])
        strawberry_invoice = strawberry_invoice + total
print(strawberry_invoice)

cupcake_invoice = []
cupcake_invoice.append(strawberry_invoice)
cupcake_invoice.append(chocolate_invoice)
cupcake_invoice.append(vannila_invoice)
print(cupcake_invoice)

from matplotlib import pyplot as plt
import numpy as np

plt.pie(cupcake_invoice)
